...
from utils.mantle_utils import get_features
...
def main():
    ...
    parser = argparse.ArgumentParser(description="MaNtLE Explanations Generation", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    for sub_idx in range(args.num_subsets):
        ...
        with torch.no_grad():
            outputs = model.generate(input_ids, decoder_start_token_id=start_token, 
        ...
        output_txts.append(output_txt[0])

    # Print out the different texts from the model
    with open(f'data/{args.dataset_name}/mantle_subsets/{args.model_name}/mantle_explanations.txt', 'w') as f:
