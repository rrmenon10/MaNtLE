...
def perturbation(data, dataset):
    NUM_PERTUBATIONS = 10*data.shape[0] # let's do more perturbations than datapoints and then give requisite amount

    perturbed_data = []
    while len(perturbed_data) < NUM_PERTUBATIONS:
        if dataset.class_names == ['<=50K', '>50K']: # adult dataset
            num_features = data.shape[1]
            num_features_to_perturb = np.random.choice(range(1, num_features))
            perturbing_features = np.random.choice(range(num_features), size=num_features_to_perturb)

            new_data_idx = np.random.choice(data.shape[0])
            new_data = copy.deepcopy(data[new_data_idx])
            for perturb_feat in perturbing_features:
                if perturb_feat in dataset.categorical_features:
                    new_fval = np.random.choice(dataset.categorical_names[perturb_feat])
                elif perturb_feat==3:
                    new_fval = str(np.random.choice(range(10000))) # 10000 should be maxmimum capital gain.
                elif perturb_feat==4:
                    new_fval = str(np.random.choice(range(80))) # 80 should be maxmimum working hrs.
                new_data[perturb_feat] = new_fval

            if (new_data != data[new_data_idx]).any():
                perturbed_data.append(new_data)
        elif dataset.class_names == ['0', '1']: # travel-insurance dataset
            num_features = data.shape[1]
            num_features_to_perturb = np.random.choice(range(1, num_features))
            perturbing_features = np.sort(np.random.choice(range(num_features), size=num_features_to_perturb, replace=False))
            ...
            for perturb_feat in perturbing_features:
                if perturb_feat in dataset.categorical_features:
                    ...
                    new_fval = str(np.random.choice(range(30,60))) # 30->60 age group
                elif perturb_feat==2:
                    new_fval = str(np.random.choice(range(2,6))) # 2->6 family members
                new_data[perturb_feat] = new_fval
            
            if (new_data != data[new_data_idx]).any():
                perturbed_data.append(new_data)
        elif dataset.class_names == ['No more', 'Re-arre']:
            num_features = data.shape[1]
            num_features_to_perturb = np.random.choice(range(1, num_features))
            perturbing_features = np.sort(np.random.choice(range(num_features), size=num_features_to_perturb, replace=False))
            ...
            raise NotImplementedError('Not implemented perturbations for this dataset!')

    perturbed_data = [perturbed_data[idx] for idx in np.random.choice(range(len(perturbed_data)), size=data.shape[0])]
...
def map_array_values(array, value_map):
    ...
    for src, target in value_map.items():
        if isinstance(ret[0], np.bytes_):
            ret = np.array([r.decode() for r in ret])
        ret[ret == src] = target
    return ret
...
def load_dataset(dataset_name, balance=False, discretize=True, dataset_folder='./'):
    ...
    if dataset_name == 'adult':
        ...
        features_to_use = [1, 3, 5, 10, 12]
        categorical_features = [1, 3, 5, 10]
        education_map = {
        ...
        features_to_use = [0, 1, 4, 10, 12]
        categorical_features = [0, 1, 4, 10, 12]
        def violations_fn(x):
            x = x.astype(float)
        ...
        dataset = load_csv_dataset(
            ...
            features_to_use=features_to_use, balance=balance, categorical_features=categorical_features,
        ...
        # Age, AnnualIncome, Number of Family Members, Frequent Flyer, EverTravelledAbroad
        features_to_use = [0, 3, 4, 6, 7]
        categorical_features = [3, 6, 7]
        ...
        dataset = load_csv_dataset(
                            os.path.join(dataset_folder, 'travel_insurance/TravelInsurancePrediction.csv'),
                            -1, ',', features_to_use=features_to_use, feature_names=feature_names,
                            categorical_features=categorical_features, discretize=discretize,
                            feature_transformations=transformations, balance=balance)
...
def load_csv_dataset(data, target_idx, delimiter=',',
    ...
    try:
        data = np.genfromtxt(data, delimiter=delimiter, dtype=str) # , dtype='|S128'
    except:
        import pandas
...
def load_data(dataset_name):
    ...
    if dataset_name == 'adult':
        ...
        dataset = load_dataset('adult', balance=True, dataset_folder=dataset_folder, discretize=False)
    elif dataset_name == 'recidivism':
        dataset_folder = 'datasets/'
        dataset = load_dataset('recidivism', balance=True, dataset_folder=dataset_folder, discretize=False)
    elif dataset_name == 'travel_insurance':
        dataset_folder = 'datasets/'
        dataset = load_dataset('travel_insurance', balance=True, dataset_folder=dataset_folder, discretize=False)
    else:
        raise NotImplementedError('This dataset loader has not been implemented!')
    
    train_dir = os.path.join('data', dataset_name ,'train')
    dev_dir = os.path.join('data', dataset_name ,'dev')
    test_dir = os.path.join('data', dataset_name,'test')
    model_dir = os.path.join('data', dataset_name,'models')
    ...
    pickle.dump(dataset.labels_train, open(os.path.join(train_dir,'gt_labels') + '.pkl','wb'))
    pickle.dump(dataset.validation, open(os.path.join(dev_dir,'data') + '.pkl','wb'))
    pickle.dump(dataset.labels_validation, open(os.path.join(dev_dir,'gt_labels') + '.pkl','wb'))
    pickle.dump(dataset.test, open(os.path.join(test_dir,'data') + '.pkl','wb'))
    pickle.dump(dataset.labels_test, open(os.path.join(test_dir,'gt_labels') + '.pkl','wb'))
